...
Push and Swap edustaa rinnakkaisen reitinhaun uutta sukupolvea (2011) ja on verrattaen yksinkertainen toimintaperiaatteeltaan. Nimensä mukaisesti algoritmissa on kaksi keskeistä operaatiota, \textit{Push} ja \textit{Swap}, sekä kaksi apuoperaatiota, \textit{Clear} ja \textit{Resolve}. Push-operaatio liikuttaa toimijaa $r$ kohti maalia. Mikäli maaliin ei päästä jonkun toisen toimijan tukkiessa tien, push yrittää työntää tiellä olevat toimijat pois edestä näiden viereisiin vapaisiin solmuihin ja jatkaa sitten maaliin. Mikäli vapaata tilaa ei ole, tai pitäisi työntää jo omiin maaleihinsa löytäneitä toimijoita sivuun, otetaan Swap-operaatio käyttöön. Swap ottaa käsiteltäväksi myös toimijan $s$, joka on suoraan $r$:n edessä lyhimmällä reitillä kohti maalia. Nyt $r$ ja $s$ pyrkivät työntämään kaikkia edessä olevia toimijoita pois tieltä, vaikka ne olisivat jo maaliin päässeitä. Pyrkimys on työntyä $r$:n maalisolmuun asti, vapauttaa sen vierestä kaksi solmua Clear-operaatiolla paikanvaihdosta varten, ja tämän jälkeen järjestää edestä työnnetyt toimijat takaisin alkuperäisille paikoilleen Resolve-operaatiolla käänteisessä järjestyksessä~\cite{luna2011push}.

\begin{figure}
\captionsetup{width=1\linewidth}
\includegraphics[scale=1]{push_swap}
\centering
\caption{Yksinkertainen Swap-operaatio Push and Swap-algoritmilla, missä tarkoitus on vaihtaa 1:n ja 2:n paikkaa~\cite{luna2011push}.}
\label{push_swap}
\end{figure}
...
Reitinhaku ja sen erityistapaus rinnakkainen reitinhaku pysyvät avoimina ongelmina vuosikymmeniä kestäneestä tutkimuksesta huolimatta. Syynä tähän on ensisijaisesti algoritmien vaihteleva soveltuvuus eri tarpeisiin, ja toisaalta kasvaneet tehokkuusvaatimukset etenkin tietokonepelien saralla. Kehitys on toisaalta kulkenut kohti algoritmeja, jotka hyödyntävät useampitasoista hierarkiaa vähentämään laskentaresurssien tarvetta abstraktion kautta, sekä toisaalta kohti kokonaan uudenlaisia ajattelumalleja, kuten \textit{Push and Swap} tai \textit{Push and Rotate}.
...
Ohjelmoijan vastuulle jääkin pohtia topologiaa, toimijoiden lukumäärää ja laskentanopeuden tarvetta reitinhakualgoritmia valitessaan. Kaikkiin tilanteisiin sopivaa yleisalgoritmia ei ole olemassa, eikä sellaista kenties ole mahdollista toteuttaakaan.
