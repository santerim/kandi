...
Tyypillisesti kenttä muodostetaan soluista (cell), joista toisinaan käytetään myös nimitystä laatta (tile). Solut ovat käytännössä joko kolmioita, neliöitä tai kuusikulmioita, sillä nämä ovat ainoat monikulmiot joita yhdistelemällä voidaan jokin alue kattaa yhtenäisesti ilman väliin jääviä aukkoja. Solut joissa pelihahmot voivat liikkua muodostavat reitinhaussa käytettävän verkon (graph) solmut~\cite{botea98}.

Kyseessä on siis verkko \texttt{G=(V,E)}, missä \texttt{V} (vertex) on joukko solmuja ja \texttt{E} (edge) joukko solmuja yhdistäviä kaaria. Liikkuminen voi tapahtua joko solmusta toiseen, tai sitten solmut voivat edustaa kiintopisteitä joiden läheisyydessä voidaan liikkua ilman erillistä reitinhakua. Solmut voivat myös toimia esteinä, jolloin niihin liikkuminen on estetty. Tämä voi myös olla vain väliaikainen tila esimerkiksi jonkun toisen toimijan ollessa liikkumisen tiellä. Molemmissa tapauksissa täytyy luonnollisesti löytää reitti esteen ympäri. Topologia voi siis olla dynaaminen, eli sen rakenne saattaa muuttua kesken reittien läpikäymisen, mikä asettaa omat haasteensa reitinhaulle~\cite{botea98, algfoor15}.
...
Reitinetsintä jakaantuu kahteen vaiheeseen: Toimintaympäristöstä muodostetaan ensin yksinkertaistettu malli, minkä jälkeen sitä käydään läpi jollakin algoritmilla halutun reitin löytämiseksi. Algoritmia ohjaa heuristiikkafunktio, jolla arvioidaan etäisyyttä kohteeseen.

Peliteollisuuden standardialgoritmi tähän tarkoitukseen on nimeltään A* (eli A-star tai A-tähti), josta on kehitetty lukuisia eri variantteja eri toimintaympäristöjä ja tarpeita silmälläpitäen~\cite{sharon15, botea04}.
...
Algoritmin läpikäymille solmuille lasketaan arvo kaavalla \newline\newline\texttt{f(n) = g(n)+h(n)}\newline\newline missä \texttt{g(n)} on lyhin tunnettu reitti lähtösolmusta solmuun \texttt{n} ja \texttt{h(n)} on heuristinen arvio etäisyydestä maalisolmuun~\cite{sharon15, stout}. Näin jokaisen läpikäydyn solmun kohdalla tiedetään sille lasketusta arvosta kuinka suoralla reitillä kohteeseen ollaan. Solmuille voidaan myös asettaa edellämainittuun kaavaan lisättävä arvo tekemään siihen siirtymisestä hintavampaa ja näin mallintaa hitaampaa kulkuyhteyttä kahden paikan välillä.\newline

[muotoilu, lähde, kieli/käännös, tyyli?]
\newline

\noindent\texttt{// A*-algoritmi pseudokoodina}\newline

\noindent\texttt{initialize the open list}\newline
\texttt{initialize the closed list}\newline
\texttt{put the starting node on the open list (you can leave its f at zero)}\newline
\newline
\texttt{while the open list is not empty}\newline
\indent\texttt{    find the node with the least f on the open list, call it "q"}\newline
\indent\texttt{    pop q off the open list}\newline
\indent\texttt{    generate q's 8 successors and set their parents to q}\newline
\indent\texttt{    for each successor}\newline
\indent\indent\texttt{    	if successor is the goal, stop the search}\newline
\indent\indent\texttt{        successor.g = q.g + distance between successor and q}\newline
\indent\indent\texttt{        successor.h = distance from goal to successor}\newline
\indent\indent\texttt{        successor.f = successor.g + successor.h}\newline
\newline
\indent\indent\texttt{        if a node with the same position as successor is in the OPEN}\newline
\indent\indent\texttt{        list}\newline
\indent\indent\indent\texttt{            which has a lower f than successor, skip this successor}\newline
\indent\indent\texttt{        if a node with the same position as successor is in the CLOSED}\newline
\indent\indent\texttt{        list}\newline
\indent\indent\indent\texttt{            which has a lower f than successor, skip this successor}\newline
\indent\indent\texttt{        otherwise, add the node to the open list}\newline
\indent\texttt{    end}\newline
\indent\texttt{    push q on the closed list}\newline
\texttt{end}\newline


\section{Rinnakkainen reitinhaku}

Videopeleissä on yleensä useita, jopa satoja tai tuhansia, eri toimijoita, joille on löydettävä reitit kohteisiinsa. Tällainen monen toimijan rinnakkainen reitinhaku (multi-agent pathfinding, MAPF) tuo yksittäisen toimijan reitinhakuun verrattuna uusia ongelmia. On ratkaistava muun muassa sallitaanko reittien risteäminen, voiko kaksi tai useampi toimijaa olla samaan aikaan samassa paikassa ja tuleeko liikkumista porrastaa odottamalla että reitti edessä vapautuu~\cite{erdem}. Näiden ongelmien ratkaisemiseen on kehitetty useita eri algoritmeja, joista useimmat hyödyntävät A*-algoritmia~\cite{sharon15}.
...
\subsection{Multi-agent Rapidly-exploring Random Tree eli MA-RRT*}
