...
Tässä algoritmissa on kuitenkin joitakin ongelmia. Swap-operaatio vaatii toimiakseen sellaisen solmun, jolla on vähintään kolme naapuria, mikä rajoittaa topologioita missä Push and Swap voi toimia. Swap voi myös johtaa laittomaan (illegal) tilaan, mikäli se saa aikaan useita muita Swap-operaatioita ahtaassa tilassa. Graafissa esiintyvä kannas (isthmus) voi johtaa lukkiutumiseen. Clear-operaatio ei osaa käsitellä tilannetta, missä siirtämällä tiellä olevaa toimijaa kahdesti saataisiin riittävästi tilaa paikanvaihtoa varten.

%Swap ottaa käsiteltäväksi myös toimijan $s$, joka on suoraan $r$:n edessä lyhimmällä reitillä kohti maalia. Nyt $r$ ja $s$ pyrkivät työntämään kaikkia edessä olevia toimijoita pois tieltä, mukaanlukien jo omiin maaleihinsa päässeet. Pyrkimys on työntyä $r$:n maalisolmuun asti, vapauttaa sen vierestä kaksi solmua Clear-operaatiolla paikanvaihdosta varten, ja tämän jälkeen järjestää edestä työnnetyt toimijat takaisin alkuperäisille paikoilleen Resolve-operaatiolla käänteisessä järjestyksessä~\cite{luna2011push, de2013push}.
...
Tämä on Push and Swap:iin perustuva algoritmi vuodelta 2013. 
